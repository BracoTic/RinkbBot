...
function getAuthToken() {
  return localStorage.getItem("rinkbot_token") || "";
}
function authHeaders(extra = {}) {
  return { "Content-Type": "application/json", Authorization: `Bearer ${getAuthToken()}`, ...extra };
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (currentUser) {
    if (avatarImg && currentUser.avatar_url) {
      ...
      tmpImg.onload = () => { avatarImg.src = currentUser.avatar_url; };
      tmpImg.onerror = () => { /* mantiene ../Assets/Avatar.png como fallback */ };
      tmpImg.src = currentUser.avatar_url;
    }
  ...
  // ======================
  const MAX_LOCAL_HISTORY = 100; // ~50 intercambios por sesión activa
  ...
  function saveHistory(log) {
    const trimmed = log.length > MAX_LOCAL_HISTORY ? log.slice(-MAX_LOCAL_HISTORY) : log;
  ...
  function saveVoiceHistory(log) {
    const trimmed = log.length > MAX_LOCAL_HISTORY ? log.slice(-MAX_LOCAL_HISTORY) : log;
    localStorage.setItem(VOICE_STORAGE_KEY, JSON.stringify(trimmed));
  }
  ...
  async function loadServerConfig() {
    ...
    try {
      const res = await fetch(CONFIG_ENDPOINT, { headers: authHeaders() });
  ...
  async function deleteChat(idChat) {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/chats/${idChat}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
  ...
  // SpeechRecognition requiere contexto seguro (HTTPS o localhost).
  // En HTTP sobre IP de red local Chrome lo deshabilita sin lanzar error visible.
  const isSecureContext = window.isSecureContext;
  const sttSupported = !!SpeechRecognition && isSecureContext;

  let recognizer = null;
  let isRecording = false;

  if (!isSecureContext && voiceHintEl) {
    voiceHintEl.textContent = "⚠️ El micrófono requiere HTTPS o localhost. Accede desde localhost:3000 o activa la excepción en chrome://flags/#unsafely-treat-insecure-origin-as-secure";
    if (recordBtn) {
      recordBtn.disabled = true;
      recordBtn.title = "Requiere contexto seguro (HTTPS o localhost)";
    }
  } else if (!SpeechRecognition && voiceHintEl) {
  ...
  if (sttSupported) {
    ...
    recognizer.onerror = (e) => {
      console.error("Error STT:", e.error, e);
      isRecording = false;
      if (recordBtn) recordBtn.textContent = "🎙️ Tocar para hablar";

      const mensajes = {
        "not-allowed":  "⚠️ Permiso de micrófono denegado. Permite el acceso en la barra del navegador.",
        "no-speech":    "🔇 No se detectó voz. Intenta de nuevo.",
        "network":      "⚠️ Error de red en el reconocimiento de voz.",
        "audio-capture":"⚠️ No se encontró micrófono. Verifica que esté conectado.",
        "aborted":      null, // silencioso — el usuario canceló
      };
      const msg = mensajes[e.error];
      if (msg && voiceHintEl) voiceHintEl.textContent = msg;
      else if (msg === undefined && voiceHintEl) voiceHintEl.textContent = `⚠️ Error: ${e.error}`;
    };
  ...
  if (recordBtn && sttSupported) {
    recordBtn.addEventListener("click", () => {
      try {
        ...
        console.error("Error al iniciar/detener STT:", err.name, err.message);
        isRecording = false;
        recordBtn.textContent = "🎙️ Tocar para hablar";
        if (voiceHintEl) voiceHintEl.textContent = `⚠️ No se pudo iniciar el micrófono: ${err.message}`;
      }
    });
  }
  ...
  function renderChatThread(containerEl, messages) {
    ...
    messages.forEach((m) => {
      ...
      if (m.role === "bot") {
        bubble.classList.add("md-content");
        bubble.innerHTML = renderBotText(m.text);
      } else {
