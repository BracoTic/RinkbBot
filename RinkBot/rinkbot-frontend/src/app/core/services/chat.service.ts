...
export class ChatService {
   ...
   */
  sendMessage(text: string, file?: File, imagenPreview?: string): Observable<string> {
    ...
    const request$ = file
      ? (() => {
          ...
          return this.http.post<{ response: string }>(`${this.base}/chat`, form);
        })()
      : this.http.post<{ response: string }>(`${this.base}/chat`, { message: text });
