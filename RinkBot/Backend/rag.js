...
// Límite: 200 entradas (~1.2 MB máx). TTL: 5 min.
// Limitación: se borra al reiniciar el proceso. No apta para múltiples réplicas.
const _embCache = new Map();
const EMB_CACHE_TTL_MS = 5 * 60 * 1000;
const EMB_CACHE_MAX = 200;

function _cacheGet(text) {
  const entry = _embCache.get(text);
  if (!entry) return null;
  if (Date.now() > entry.exp) { _embCache.delete(text); return null; }
  return entry.vec;
}

function _cacheSet(text, vec) {
  if (_embCache.size >= EMB_CACHE_MAX) {
    _embCache.delete(_embCache.keys().next().value); // evict más antiguo
  }
  _embCache.set(text, { vec, exp: Date.now() + EMB_CACHE_TTL_MS });
}
...
export async function embed(text) {
  ...
  const r = await getOpenAIClient().embeddings.create({ model: EMB_MODEL, input: text });
