...
| Tecnología | Versión | Uso |
|---|---|---|
| Angular | 19+ | Framework SPA — componentes standalone, Signals, lazy loading por ruta |
| TypeScript | 5.7+ | Lenguaje principal del frontend |
...
| marked.js | ^15 | Renderizado de Markdown en respuestas del bot |
| Vitest | — | Runner de tests unitarios (vía `@angular/build:unit-test`) |
...
En desarrollo, el servidor de Angular (`ng serve`) incluye un proxy que redirige `/api/*` al backend Express en el puerto 3000, por lo que ambos procesos corren en paralelo (ver sección [Desarrollo local](#desarrollo-local)).
...
> Los pasos de configuración de credenciales y variables de entorno se numeran a continuación como **5** y **6** (antes eran **3** y **4**).

### 5. Configurar el archivo de credenciales de Google Drive
...
Ver sección detallada [Desarrollo local](#desarrollo-local) más abajo.
...
# Expone /api/*, /health y (en producción) el frontend estático
...
Esto significa que `fetch('/api/chat')` desde `localhost:4200` llega al backend Express en `localhost:3000` sin necesidad de cambiar ninguna URL ni deshabilitar CORS durante el desarrollo.

La variable `environment.apiBase` vale `'/api'` tanto en `environment.ts` (dev) como en `environment.prod.ts` (producción). El interceptor JWT adjunta `Authorization: Bearer <token>` a todas las peticiones hacia el mismo origen.
...
└── rinkbot-frontend/            # Aplicación Angular
    ...
    ├── tsconfig.app.json        # TypeScript para compilación de producción
    ├── tsconfig.spec.json       # TypeScript para tests (Vitest)
    ├── package.json
    │
    ├── src/
    │   ├── main.ts              # Bootstrap de la aplicación Angular
    │   ├── styles.scss          # Estilos globales, variables SCSS, reset
    │   ├── app/
    │   │   ├── app.config.ts    # provideRouter, provideHttpClient, interceptores
    │   │   ├── app.routes.ts    # Rutas lazy: /, /login, /chat
    │   │   │
    │   │   ├── core/
    │   │   │   ├── services/
    │   │   │   │   ├── auth.service.ts    # Login, logout, JWT en localStorage
    │   │   │   │   ├── chat.service.ts    # Signals de mensajes, historial DB
    │   │   │   │   └── voice.service.ts  # SpeechRecognition + SpeechSynthesis
    │   │   │   ├── guards/
    │   │   │   │   └── auth.guard.ts     # Protege /chat — redirige a /login si no hay token
    │   │   │   ├── interceptors/
    │   │   │   │   └── jwt.interceptor.ts # Adjunta Bearer token a peticiones internas
    │   │   │   └── models/
    │   │   │       ├── user.model.ts
    │   │   │       ├── message.model.ts
    │   │   │       └── chat.model.ts
    ...
    │   │   │   └── chat/          # Aplicación principal: chat, voz, historial (ruta /chat)
    │   │   │
    │   │   └── shared/
    │   │       └── components/    # Componentes reutilizables (loading, etc.)
    │   │
    │   ├── assets/               # Imágenes corporativas (logos, avatares, fondos)
    │   └── environments/
    │       ├── environment.ts     # apiBase: '/api' (dev)
    │       └── environment.prod.ts # apiBase: '/api' (prod)
    │
    └── dist/                    # Build de producción (generado — no commiteado)
        └── browser/             # Archivos estáticos servidos por Express
...
1. ENTRADA
   Usuario abre http://tudominio.com
       ...
       → AuthGuard: si ya hay token en localStorage → navega directamente a /chat
       → Si no → carga componente Home (splash con branding corporativo)
       → Botón "Iniciar sesión" → Angular Router navega a /login

2. AUTENTICACIÓN
   Usuario ingresa credenciales en /login
       → AuthService.login(): POST /api/login { usuario, password }
       → JwtInterceptor no adjunta token (petición pública)
       → Backend: consulta public.persona con pgcrypto.crypt()
       → Si válido: genera JWT, devuelve { ok, user, token }
       → AuthService.login() guarda en localStorage: rinkbot_token, rinkbot_user
       → Angular Router navega a /chat

3. CHAT
   Usuario escribe/habla un mensaje en /chat
       → ChatService.sendMessage(): POST /api/chat con { message }
       → JwtInterceptor adjunta automáticamente Authorization: Bearer <token>
       ...
       → OpenAI gpt-4o-mini con contexto + pregunta → respuesta { reply, response, sources }
       → ChatService actualiza Signal de mensajes → componente re-renderiza vía OnPush
       → marked.js renderiza Markdown; VoiceService reproduce TTS si está activo
...
El diseño apunta a **un único contenedor Node.js** que sirve tanto el frontend Angular compilado como la API, conectándose a servicios externos (Supabase, OpenAI, Google Drive).
...
1. **Stage `builder`** — instala dependencias del frontend y ejecuta `ng build --configuration production`. Output: `dist/browser/`.
2. **Stage `runtime`** — imagen Alpine limpia con solo las dependencias de producción del backend. Copia `Backend/` y el `dist/` del stage anterior. Express los sirve desde el mismo proceso en el puerto 3000.

No se requiere servidor de frontend separado (Nginx, Caddy, etc.) en producción — Express lo maneja directamente.
...
### Partes incompletas / sin implementar en el frontend Angular
Los siguientes paneles del rail lateral están declarados en el componente `Chat` pero **sin funcionalidad implementada** (muestran un placeholder):
...
*Versión del sistema: 2.0.0 — Frontend migrado de Vanilla JS a Angular 19+ (standalone components, Signals, lazy loading). Documentación generada a partir del código fuente real del repositorio.*
